...
class RouteScenario(BasicScenario):
    ...
    def __init__(self, world, config, debug_mode=0, criteria_enable=True):
        ...
        # TODO: spawna l'ego vehicle
        ego_vehicles = self._spawn_ego_vehicle(world)
        if ego_vehicles is None:
        ...
        self._build_scenarios(
            world, ego_vehicles, sampled_scenario_definitions, timeout=10000, debug=debug_mode > 0
        )
    ...
    def _spawn_ego_vehicle(self, world):
        ...
        # TODO: Crea il vehicle con il rolename 
        # TODO: Spawniamo x vehicles con f"hero{xi}"
        # TODO: n vehicles
        ego_vehicles = []
        with open("./config.json", "r") as f:
            d = json.load(f)
            n_vehicles = d["n_vehicles"]
            vehicles_to_use = None
            if "vehicles" in d:
                vehicles_to_use = d["vehicles"]
        for i in range(n_vehicles):
            ego_vehicle = CarlaDataProvider.request_new_actor('vehicle.lincoln.mkz_2020' if vehicles_to_use is None else vehicles_to_use[i],
                                                            elevate_transform,
                                                            rolename=f'hero{i}')
    ...
    def _build_scenarios(self, world, ego_vehicles, scenario_definitions, scenarios_per_tick=5, timeout=300, debug=False):
        ...
        self.list_scenarios = []
        # TODO: change all of these ego data and ego_vehicles to have car affect score
        ego_datas = []
        for i, ego_vehicle in enumerate(ego_vehicles):
            ego_data = ActorConfigurationData(ego_vehicle.type_id, ego_vehicle.get_transform(), f'hero{i}')
        ...
        for scenario_number, scenario_config in enumerate(scenario_definitions):
            scenario_config.ego_vehicles = ego_datas
    ...
    def _create_behavior(self):
        ...
        for ego_vehicle in self.ego_vehicles:
            ...
            scenario_triggerer = ScenarioTriggerer(
                ego_vehicle, self.route, blackboard_list, scenario_trigger_distance)
            behavior.add_child(scenario_triggerer)  # Tick the ScenarioTriggerer before the scenarios

        # Add the Background Activity
        behavior.add_child(BackgroundBehavior(self.ego_vehicles, self.route, name="BackgroundActivity"))
    ...
    def _create_test_criteria(self):
        ...
        for i, ego_vehicle in enumerate(self.ego_vehicles):
            # End condition
            criteria.add_child(RouteCompletionTest(ego_vehicle, route=self.route, name=f"RouteCompletionTest{i}"))

            # 'Normal' criteria
            criteria.add_child(OutsideRouteLanesTest(ego_vehicle, route=self.route, name=f"OutsideRouteLanesTest{i}"))
            criteria.add_child(CollisionTest(ego_vehicle, name=f"CollisionTest{i}"))
            criteria.add_child(RunningRedLightTest(ego_vehicle, name=f"RunningRedLightTest{i}"))
            criteria.add_child(RunningStopTest(ego_vehicle, name=f"RunningStopTest{i}"))
            criteria.add_child(MinimumSpeedRouteTest(ego_vehicle, self.route, checkpoints=4, name=f"MinimumSpeedRouteTest{i}"))

            # These stop the route early to save computational time
            # TODO: Offroad
            criteria.add_child(InRouteTest(
                ego_vehicle, route=self.route, offroad_max=30, terminate_on_failure=True, name=f"InRouteTest{i}"))
            criteria.add_child(ActorBlockedTest(
                ego_vehicle, min_speed=0.1, max_time=180.0, terminate_on_failure=True, name=f"ActorBlockedTest{i}")
            )
