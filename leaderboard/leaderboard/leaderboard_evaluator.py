...
class LeaderboardEvaluator(object):
    ...
    def __init__(self, args, statistics_manager):
        ...
        self.client1 = carla.Client(args.host, args.port)
        if args.timeout:
        ...
        # Load agent
        # TODO: this is the one that loads the module for the autonomous vehicle
        # TODO: going to move it below when the agent is actually created
        """        
        self.modules = []
        with open("./config.json", "r") as f:
            n_vehicles = json.load(f)["n_vehicles"]
        for index in range(n_vehicles):
            module_name = os.path.basename(args.agent).replace(".py", f"{index}.py").split('.')[0]
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        try:
            ...
            # TODO: questo si occupa anche di generare l'ego vehicle
        ...
        try:
            # TODO: This creates an instance of basic_autonomous_agent
            with open("./config.json", "r") as f:
                n_vehicles = json.load(f)["n_vehicles"]
            self.agent_instances = []
            for i in range(n_vehicles):
                # TODO: we are going to set the AI here now
                # __import__
                module_name = os.path.basename(args.agent).replace(".py", f"_{i}.py").split('.')[0]
                sys.path.insert(0, os.path.dirname(args.agent))
            ...
            if not self.sensors:
                self.sensors = self.agent_instances[0].sensors()
                track = self.agent_instances[0].track
        ...
        try:
            ...
            # TODO: this gives the agent to the scenario_manager to run
            self.manager.load_scenario(config, scenario, self.agent_instances, config.repetition_index)
