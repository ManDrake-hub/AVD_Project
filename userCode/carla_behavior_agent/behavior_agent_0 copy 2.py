...
class BehaviorAgent(BasicAgent):
    ...
    def move_loc_with_offset(self, wp, offset):
        ego_transform_pred = wp.transform
    ...
    def run_step(self, debug=False):
        ...
        prop_list = self._world.get_actors().filter("*static.prop.[!mesh][!debris]*")
        ...
        while step < max_steps:
            ...
            if not overtaking:
                if (self.check_occupied(ego_wp, transform_list, "normal") and
                    ...
                    draw_point(self._world, ego_loc_pred, color=(128, 255, 0, 255), life_time=self.time_step + 0.01)
                    draw_point(self._world, transform.location, color=(0, 255, 128, 255), life_time=self.time_step + 0.01)
        ...
        for i in range(len(offsets)):
            if i == 0:
                ...
                wp, _ = self._local_planner.get_incoming_waypoint_and_direction(self.get_ego_distance_from_step(0, i-1))
            draw_point(self._world, self.move_loc_with_offset(wp, offsets[i]), color=(255, 0, 0, 255), life_time=self.time_step + 0.01)
