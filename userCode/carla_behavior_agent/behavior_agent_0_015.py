...
class BehaviorAgent(BasicAgent):
    ...
    def check_vehicles(self, ego_wp, locations, lane: str, show=False):
        ...
        for vehicle, location, rotation in locations:
            ...
            if ego_pol.intersects(pol):
                if show:
                    self._world.debug.draw_box(carla.BoundingBox(location, extent), 
                            rotation, life_time=0.06, color=carla.Color(255, 0, 255))
    ...
    def run_step(self, debug=False):
        ...
        junction = False
        frontal_step = None
        steps_to_consider_offset = 21
        steps_to_consider_speed = 21
        base_max_steps = 21
        print("#######################################################")
        ego_loc_preds = []
        ...
        while step < max_steps:
            ...
            draw_point(self._world, ego_wp.transform.location, color=(255, 0, 0, 255), life_time=self.time_step + 0.01)
            ...
            # Speed proposal
            #############################
            frontal_vehicles = self.check_vehicles(ego_wp, transform_list, "normal", show=True)
            if step < steps_to_consider_speed and speed_front is None and frontal_vehicles:
                ...
                if step < 3:
                   speed_front = speed_front 
                    ...
                    speed_front = max(1, speed_front / 2)
                elif step < int(steps_to_consider_speed/2):
                    speed_front = max(3, speed_front / 2)
                else:
            ...
            # Right and Left free
            #############################
            if step < 9 and self.check_occupied(ego_wp, transform_list, "overtake"):
                left_free = False

            if step < 9 and self.check_occupied(ego_wp, transform_list, "normal"):
                normal_free = False

            if step < 9 and self.check_occupied(ego_wp, transform_list, "right"):
                right_free = False
            #############################
        ...
        if any([x < 0.0 for x in offsets]) and ((left_free and not self.prev_offset < 0.0) or self.prev_offset < 0.0) and frontal_step < 3 and speed_front < 3:
