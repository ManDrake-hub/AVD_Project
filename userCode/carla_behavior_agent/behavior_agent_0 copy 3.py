...
class BehaviorAgent(BasicAgent):
    ...
    def run_step(self, debug=False):
        ...
        while step < max_steps:
            ...
            if -1 < step < steps_to_consider_speed and speed_front is None and frontal_vehicles:
            ...
            if step < 2 and self.check_occupied(ego_wp, transform_list, "normal"):
                normal_free = False
        ...
        can_stop = self.can_stop_overtake(ego_vehicle_loc) if not junction else not self.prev_offset < 0.0

        if any([x < 0.0 for x in offsets]) and ((left_free and can_stop and ego_vehicle_speed < 1.0) or not can_stop) and not normal_free:
            offset_final = min(offsets)
        elif any([x > 0.0 for x in offsets]) and ((right_free and not can_stop) or can_stop):
        ...
        if offset_final < 0:
            ...
            return control
        elif speed_front is not None and can_stop:
            if speed_front <= 0.1:
                if junction:
                    speed_front = 0.1
                else:
