...
GlobalRoutePlanner = __import__("global_route_planner_" + __file__.replace(".py", "").split("_")[-1]).GlobalRoutePlanner
LocalPlanner = __import__("local_planner_" + __file__.replace(".py", "").split("_")[-1]).LocalPlanner
...
class BasicAgent(object):
    ...
    def __init__(self, vehicle, opt_dict={}, map_inst=None, grp_inst=None):
        ...
        """
        ################################################################
        # TODO: save the index and load the global planner and local planner with the correct index
        self.index = self.index = int(__file__.replace(".py", "").split("_")[-1])
        ################################################################
