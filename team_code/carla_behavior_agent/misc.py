...
def get_sensors_locations(ego_vehicle, location_list, sensors, max_distance=60):
    """
        :param ego_vehicle: ego vehicle
    ...
    def dist(location): return location.distance(ego_vehicle_loc)
    vehicles = [(vehicle, *compute_magnitude_angle_with_sign(location, ego_vehicle_loc, ego_vehicle_transform.rotation.yaw)) for vehicle, location in location_list 
    ...
    for sensor in sensors:
        ...
        for v in vehicles:
            ...
            if (sensor_range_min <= distance <= sensor_range_max and sensor_angle_min <= angle <= sensor_angle_max):
                ...
                sensors_result[sensor_id]["speed_deltas"].append((vehicle.get_velocity().length() / (ego_vehicle.get_velocity().length() + 1e-5)) - 1.0)
