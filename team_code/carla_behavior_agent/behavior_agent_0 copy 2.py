...
class BehaviorAgent(BasicAgent):
    ...
    def get_vehicles_on_opposite_lane(self, vehicle_list, step):
        ...
        for vehicle in vehicle_list:
            ...
            if self.get_lane(v_loc) == -1 * self.get_lane(ego_wp.transform.location):
                print(self.get_lane(v_loc), self.get_lane(ego_wp.transform.location))
    ...
    def run_step(self, debug=False):
        ...
        # Aree di interesse
        sensors = [("right", 0, 5, 10, 170), ("left", 0, 5, -160, -20), ("front", 0, 5, -5, 5), ("front_long", 0, 15, -15, 15)]
        offsets = []
        ...
        for step in range(-1, 14):
            ...
            if sensors_result["left"] and (self.get_lane(self.predict_ego_data(0.0, step)[1] if step != -1 else ego_vehicle_loc) != self.get_lane([x[1] for x in location_list if x[0] == sensors_result["left"][0][0]][0])):
                ...
                if _prev_offset == 0.0:
                    print("\n\ncheck", step)
                    if sensors_result["front_long"]:
                        vehicles_in_front = [x[0] for x in sensors_result["front"]]
                        ...
                        print("front", sensors_result["front"])
                        print("front_long", sensors_result["front_long"])

                        if len([x[0] for x in sensors_result["front_long"] if x[0] not in vehicles_in_front]) > 0:
                            emergency_stop = True
                ...
                offset = 0.0
                # if _prev_offset < 0:
                #     print("opposite", len(self.get_vehicles_on_opposite_lane(vehicle_list, step)), len([x[0] for x in sensors_result["front_long"]]))
                #     for vehicle in self.get_vehicles_on_opposite_lane(vehicle_list, step):
                #         if vehicle not in [x[0] for x in sensors_result["front_long"]]:
                #             continue
                #         print("check")
                #         if misc.distance_vehicle_locations(ego_loc_pred, vehicle.get_location()) <= 20:
                #             emergency_stop = True
                #             print("Emergency", step, misc.distance_vehicle_locations(ego_loc_pred, vehicle.get_location()))

            _prev_offset = offset
            if step < 7:
                offsets.append(offset)

        offset_final = misc.exponential_weighted_average(offsets, 0.35)
        if emergency_stop:
            speed_final = 0.0
            offset_final = 0.0
