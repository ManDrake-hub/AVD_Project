...
class BasicAgent(object):
    ...
    def _affected_by_traffic_light(self, lights_list=None, max_distance=None):
        ...
        # prende una di default
        if not max_distance:
            max_distance = self._base_tlight_threshold

        # se esiste un traffic light a cui ci siamo già fermati (se esiste ed è ancora rosso, se esiste ed è verde)
        ...
        for traffic_light in lights_list:
            ...
            if trigger_wp.transform.location.distance(ego_vehicle_location) > max_distance:
                continue
            # se è sulla stessa strada
            if trigger_wp.road_id != ego_vehicle_waypoint.road_id:
                continue
            # prima consideriamo la distanza, questo consente di capire la posa (per evitare di considerare semafori dietro di noi)
    ...
    # DA MODIFICARE:
    # Si può modificare gestendo casomai il fatto che se impatta sulla traiettora in quel momento, potrebbe non impattarlo dopo perchè si muove
    # quindi potremmo gestire la proiezione del movimento dell'oggetto sulla nostra traiettoria
    def _vehicle_obstacle_detected(self, vehicle_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        ...
        # prende il waypoint in cui stava il veicolo
        for target_vehicle in vehicle_list:
            target_transform = target_vehicle.get_transform()
            target_wpt = self._map.get_waypoint(target_transform.location, lane_type=carla.LaneType.Any)

            # Simplified version for outside junctions
            # se non siamo in un incrocio, quindi casi in cui non ci sono svolte 
            # se non sto nella stessa strada o stessa lane, allora prendi il prossimo waypoint 
            if not ego_wpt.is_junction or not target_wpt.is_junction:

                if target_wpt.road_id != ego_wpt.road_id or target_wpt.lane_id != ego_wpt.lane_id  + lane_offset:
                    # prende dalla coda dei waypoint dati al local planner si prende il terzo (per prendere quello un po' più avanti e non quello proprio vicino) 
                    # e [0] perchè è una tupla (waypoint, direction(descrive cosa ci devo fare tipo lane follow, change direction ecc..)) 
                    next_wpt = self._local_planner.get_incoming_waypoint_and_direction(steps=3)[0]
                    if not next_wpt:
                        continue
                    # controllo se mi interessa il veicolo che ho riconosciuto (stessa corsia o stessa strada)
                    if target_wpt.road_id != next_wpt.road_id or target_wpt.lane_id != next_wpt.lane_id  + lane_offset:
                        continue

                target_forward_vector = target_transform.get_forward_vector()
                target_extent = target_vehicle.bounding_box.extent.x
                # rear_transoform punto posteriore del veicolo che stiamo considerando
                target_rear_transform = target_transform
                target_rear_transform.location -= carla.Location(
                    x=target_extent * target_forward_vector.x,
                    y=target_extent * target_forward_vector.y,
            ...
            # costruzione del poligono sulla traiettoria e valutato massimo punto a sinistra e destra
            else:
                route_bb = []
                ego_location = ego_transform.location
                extent_y = self._vehicle.bounding_box.extent.y
                ...
                # per ogni punto del get_plan(lista di waypoints). Se la distanza è troppo lontana, non me ne frega. 
                # Quindi prendo solo i punti del plan più vicino
                for wp, _ in self._local_planner.get_plan():
                    if ego_location.distance(wp.transform.location) > max_distance:
                        break
                    ...
                    route_bb.append([p1.x, p1.y, p1.z])
                    route_bb.append([p2.x, p2.y, p2.z])
                # route_bb: per ogni waypoint vicino a me il mio massimo punto a sinsitra e destra
                # ho così un poligono della traiettoria
                if len(route_bb) < 3:
                ...
                # se mi trovo in un incrocio e il veicolo è troppo distante, vado avanti
                # se è un veicolo che deve essere considerato, prendo il bounding box del veicolo
                # di questo bounding box prendo i 4 punti (ho un nuovo poligono) 
                # e considero se il poligono si interseca con la traiettoria che devo fare 
                # quindi se devo girare a destra e ho una macchina a sinistra, non mi interessa 
                for target_vehicle in vehicle_list:
                    target_extent = target_vehicle.bounding_box.extent.x
                    if target_vehicle.id == self._vehicle.id:
                        continue
                    ...
                    if ego_polygon.intersects(target_polygon):
                        # se impatta chi è e dove si trova
                        return (True, target_vehicle, compute_distance(target_vehicle.get_location(), ego_location))
