...
def is_within_distance(target_transform, reference_transform, max_distance, angle_interval=None):
    ...
    :return: boolean
    """
    # differenza tra posizione nostra e del veicolo
    ...
    # Se non li definisco, dico che sto in collisione
    ...
    # coseno tra i due vettori quindi abbiamo l'angolo tra il vettore di orientamento del veicolo e la differenza di posizioni
    angle = math.degrees(math.acos(np.clip(np.dot(forward_vector, target_vector) / norm_target, -1., 1.)))

    # dipende dall'intenzione che abbiamo (se abbiamo un cambio corsia, gli angoli dipendono dall'impatto del veicolo dietro sulla traiettoria)
    # dipende solo dalla differenza di dove ci troviamo. Se abbiamo un veicolo in direzione opposta su un altra corsia, non ho problemi
